...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    float speed = weatherMeterKit.getWindSpeed();
    Serial.print(F("Wind speed (kph): "));
    Serial.println(weatherMeterKit.getWindSpeed(), 1);
    // adjust for the f2sflt16 range (-1 to 1)
    speed = speed / 1000;

     float direction = weatherMeterKit.getWindDirection();
     Serial.print(F("Direction (angleº): "));
     Serial.println(weatherMeterKit.getWindDirection(), 1);
     direction = direction / 1000;

    float rain = weatherMeterKit.getTotalRainfall();
    Serial.print(F("Total Rain(ml): "));
    Serial.println(weatherMeterKit.getTotalRainfall(), 1);
    // adjust for the f2sflt16 range (-1 to 1)
    rain = rain / 1000;


    uint16_t payloadSpeed = LMIC_f2sflt16(speed);
    // int -> bytes
    byte speedLow = lowByte(payloadSpeed);
    byte speedHigh = highByte(payloadSpeed);
    // place the bytes into the payload
    payload[0] = speedLow;
    payload[1] = speedHigh;

    uint16_t payloadDirection = LMIC_f2sflt16(direction);
    // int -> bytes
    byte directionLow = lowByte(payloadDirection);
    byte directionHigh = highByte(payloadDirection);
    // place the bytes into the payload
    payload[2] = directionLow;
    payload[3] = directionHigh;

    uint16_t payloadRain = LMIC_f2sflt16(rain);
    // int -> bytes
    byte rainLow = lowByte(payloadRain);
    byte rainHigh = highByte(payloadRain);
    // place the bytes into the payload
    payload[4] = rainLow;
    payload[5] = rainHigh;
